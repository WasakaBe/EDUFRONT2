...
const SchedulesView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}carreras/tecnicas`)
      ...
      .catch((error) =>
        toast.error(`Error fetching technical careers: ${error.message}`)
      )
  }, [])
  ...
  const handleSubmit = () => {
    if (selectedHorario) {
      fetch(
        `${apiUrl}horarios_escolares/update/${selectedHorario.id_horario}`,
        {
          method: 'PUT',
          headers: {
        ...
        .then((data) => {
          setHorarios(
            horarios.map((horario) =>
              horario.id_horario === data.id_horario ? data : horario
            )
          )
  ...
  const handleDelete = () => {
    if (selectedHorario) {
      fetch(
        `${apiUrl}horarios_escolares/delete/${selectedHorario.id_horario}`,
        {
          method: 'DELETE',
        }
      )
        .then((response) => {
          ...
          setHorarios(
            horarios.filter(
              (horario) => horario.id_horario !== selectedHorario.id_horario
            )
          )
  ...
  return (
    <div className="schedules-view-container">
        ...
        <tbody>
          {currentItems.map((horario) => (
            <tr key={horario.id_horario}>
              ...
              <td>
                {Array.isArray(horario.dias_horarios) &&
                horario.dias_horarios.length > 0 ? (
      ...
      <Modal
        ...
        className="modal-schedules-view"
      >
        ...
        {selectedHorario && (
          <form>
            ...
            <label>
              ...
              >
                ...
                {docentes.map((docente) => (
                  ...
                  >
                    {docente.nombre_docentes} {docente.app_docentes}{' '}
                    {docente.apm_docentes}
                  </option>
                ))}
              </select>
            </label>
              ...
              >
                ...
                {carrerasTecnicas.map((carrera) => (
                  <option
                    ...
                    value={carrera.nombre_carrera_tecnica}
                  >
                ...
                <div key={index}>
                  <label>
                    ...
                    <input
                      ...
                      value={dia.day}
                      onChange={(e) =>
                        handleDiaHorarioChange(index, 'day', e.target.value)
                      }
                    />
                  </label>
                  <label>
                  ...
                  <label>
                    ...
                    <input
                      ...
                      value={dia.endTime}
                      onChange={(e) =>
                        handleDiaHorarioChange(index, 'endTime', e.target.value)
                      }
                    />
                  </label>
                </div>
            ...
            <button
              className="save-button-schedules-view"
              type="button"
